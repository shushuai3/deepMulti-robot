...
def image_predict(image_path, model):
    ...
    original_image      = np.swapaxes(np.copy(original_image),0,1)
    image_data = original_image
    image_data = image_data[np.newaxis, ...].astype(np.float32)
    conv = model.predict(image_data)
    conf = tf.sigmoid(conv[0, :, :, 1:2])
    pos_conf_above_threshold = np.argwhere(conf > 0.23)
    ...
    for xy in pos_conf_above_threshold:
        curX = (xy[0]-0.5)*8
        curY = (xy[1]-0.5)*8
        draw.ellipse((curX-4, curY-4, curX+4, curY+4), outline ='white', width=2)
        draw.ellipse((curX-7, curY-7, curX+7, curY+7), outline ='black', width=2)
        draw.text((curX-20, curY),"{:3.2f}m".format(tf.exp(conv[0, xy[0], xy[1], 0])),fill='white', stroke_fill='black', stroke_width=1, font=font)
    image.show()
...
image_predict("./dataset/synImgs/0837.jpg", model)
image_predict("./dataset/synImgs/0970.jpg", model)
# image_predict("./dataset/realImgs/0037.jpg", model)
